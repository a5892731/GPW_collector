...
class WSE_data:
    ...
    header_oryginal = ["Walor", "Kurs (PLN)", "Zmiana (%)", "Otwarcie", "Min", "Max",
                       "Obrót (szt.)", "Obrót (PLN)", "Czas_aktualizacji"]
    header_eng = ["Value", "Rate (PLN)", "Change (%)", "Opening", "Min", "Max",
                  "Turnover (pcs.)", "Turnover (PLN)", "Update_time"]
    ...
    def __init__(self):
        ...
        self.data_table = self.translate_data(headline_data)
...
if __name__ == "__main__":
    ...
    GPW = WSE_data()
    print(GPW.header_oryginal)
    for row in GPW.data_table:
        print(row)